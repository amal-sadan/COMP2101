...
function end {
  sleepCount=0
  echo "The secret key has been found"
  exit
}

function reset {
sleepCount=$numberOfSleeps
echo "Interruption not allowed"
}
######################
...
while [ $# -gt 0 ]; do
    case $1 in
        ...
        * )
            usage
            error-exit "$1 option not recognized"
    esac
    shift
done
...
echo "please Wait the counter has been expired, exiting "
